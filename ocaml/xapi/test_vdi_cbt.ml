...
let test_data_destroy =
  ...
  let test_data_destroy_timing =
    (*      >= 0.2   start unplug 0.2  finish unplug  0.2 destroy VBD
     * |------------------|------------------|------------------| *)
    let vbd_unplug_and_destroy_in_background ~__context ~self ?(start_delay=1.0) () =
      let vbd_unplug_and_destroy () =
      ...
      in
      Thread.create vbd_unplug_and_destroy ()
    in

    (*                                  1.0                             data destroy times out
     * |---------------------------------------------------------------------------| *)
    let timeboxed_data_destroy ~__context ~timeout_message self =
      let exception Data_destroy_timeout of float * string in
      let timeout = 4.0 +. 1.0 in
      Helpers.timebox
        ~timeout
        ~otherwise:(fun () -> raise (Data_destroy_timeout (timeout, timeout_message)))
        (fun () -> Xapi_vdi._data_destroy ~__context ~self ~timeout:4.0)
    in
    ...
    let setup_test () =
      ...
      let vbd_unplug_and_destroy_in_background = vbd_unplug_and_destroy_in_background ~__context ~self:vbd in
      let timeboxed_data_destroy = timeboxed_data_destroy ~__context vDI in
      (vbd_unplug_and_destroy_in_background, timeboxed_data_destroy, vDI)
    in

    let test_data_destroy_succeeds =
      let test_data_destroy_succeeds ~data_destroy_start () =
        let vbd_unplug_and_destroy_in_background, timeboxed_data_destroy, _ = setup_test () in
        let t = vbd_unplug_and_destroy_in_background () in
        Thread.delay data_destroy_start;
        timeboxed_data_destroy ~timeout_message:"data_destroy did not finish succesfully in time";
        Thread.join t
      in
      let open OUnit in
      "test_data_destroy_succeeds" >:::
      (*        1.0    start unplug 1.0  finish unplug  1.0 destroy VBD
       * |------------------|------------------|------------------|
       *                                   4.0                             data destroy times out
       * |---------------------------------------------------------------------------| *)
      [ "test_data_destroy_succeeds_when_unplug_starts_later" >:: test_data_destroy_succeeds ~data_destroy_start:0.0
      (*        1.0    start unplug 1.0  finish unplug  1.0 destroy VBD
       * |------------------|------------------|------------------|
       *           1.5                                                 4.0                             data destroy times out
       * ............................|---------------------------------------------------------------------------| *)
      ; "test_data_destroy_succeeds_when_vbd_is_being_unplugged" >:: test_data_destroy_succeeds ~data_destroy_start:1.5
      (*        1.0    start unplug 1.0  finish unplug  1.0 destroy VBD
       * |------------------|------------------|------------------|
       *                  2.5                                                              4.0                             data destroy times out
       * ................................................|---------------------------------------------------------------------------| *)
      ; "test_data_destroy_succeeds_when_unplug_finished" >:: test_data_destroy_succeeds ~data_destroy_start:2.5
      ]
    in

    let test_data_destroy_times_out =
      let test_data_destroy_times_out ~unplug_start () =
        let vbd_unplug_and_destroy_in_background, timeboxed_data_destroy, vDI = setup_test () in
        let t = vbd_unplug_and_destroy_in_background ~start_delay:unplug_start () in
        OUnit.assert_raises
          Api_errors.(Server_error (vdi_in_use, [Ref.string_of vDI; "data_destroy"]))
          (fun () -> timeboxed_data_destroy ~timeout_message:"data_destroy did not time out");
        Thread.join t
      in
      let test_data_destroy_times_out_when_nothing_happens_to_vbd () =
        let _, timeboxed_data_destroy, vDI = setup_test () in
        OUnit.assert_raises
          Api_errors.(Server_error (vdi_in_use, [Ref.string_of vDI; "data_destroy"]))
          (fun () -> timeboxed_data_destroy ~timeout_message:"data_destroy did not time out");
      in
      ...
      (*                  2.5                      start unplug 1.0  finish unplug  1.0 destroy VBD
       * |----------------------------------------------|------------------|------------------|
       *                                   4.0                             data destroy times out
       * |---------------------------------------------------------------------------| *)
      [ "test_data_destroy_times_out_when_vbd_does_not_get_destroyed_in_time" >:: test_data_destroy_times_out ~unplug_start:2.7
      (*                               3.5                             start unplug 1.0  finish unplug  1.0 destroy VBD
       * |------------------------------------------------------------------|------------------|------------------|
       *                                   4.0                             data destroy times out
       * |---------------------------------------------------------------------------| *)
      ; "test_data_destroy_times_out_when_vbd_unplug_does_not_finish_in_time" >:: test_data_destroy_times_out ~unplug_start:3.5
